...
// Factory function
function Bills(monthName, rent, currentUnits, unitPrice) {
  return {
    month: monthName,
    rent: rent,
    units: currentUnits,
    unitPrice: unitPrice,
    electricBills: function () {
      return `Electricity bill is $${this.units * this.unitPrice}`;
    },
    totalBills: function() {
      return `Total bill is $${this.rent + (this.units * this.unitPrice)}`;
    }
  }
}

const august = Bills("August", 7000, 31, 16);
console.log(august);